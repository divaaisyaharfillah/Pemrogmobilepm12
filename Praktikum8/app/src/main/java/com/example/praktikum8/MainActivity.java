...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        notes = new ArrayList<>(); //untuk menyimpan catatan
        notesAdapter = new NotesAdapter(this, notes);
        rvSearch.setAdapter(notesAdapter); //mengatur adapter ke rv
        rvSearch.setLayoutManager(new GridLayoutManager(this, 1)); //layout manager untuk rv dengan satu kolom.

        loadData(""); //memuat semua data dari database.

        fabAdd.setOnClickListener(v -> { //klo tmbh pindah ke addactivity
    ...
    private void loadData(String query) { //untuk memuat data dari database
        notes.clear();
        Cursor cursor;
        if (query.isEmpty()) {
        ...
        if (notes.isEmpty()) { // kosong menampilkan pesan "No Data"
    ...
    @Override
    protected void onResume() { // aktivitas kembali ke tampilan depan
        super.onResume();
        loadData("");
    }
}
